...
int isDebug() {
  Rcpp::Function options("getOption");
  int l =0;
  try {
    l = as<int>(options("inDebug"));
  } catch (...) {
    ;
  }
  return l;
...
std::string format_cpp(const std::string &x, const std::string &justify, int width, int xlen) {
  std::string result = "";
...
//  while (*s) xlen += (*s++ & 0xc0) != 0x80;
  ...
  if (xlen > width) {
  	//Rprintf("nchar width > width %d %d, [%s]\n", xlen, width, x.c_str());
    xlen = width;
  }
  if (isDebug() > 0) {Rprintf("nchar > width %d %d, [%s]\n", xlen, width, x.c_str());}
...
//[[Rcpp::export]]
std::string tableExpand_cpp(CharacterVector cells, IntegerVector colsWidth, IntegerVector charWidth, CharacterVector justify, CharacterVector sepCols, std::string style) {
  ...
  if (hasLineBreak){
    ...
    // 当拆分多行时，根据 charWidth 提取每行的字符宽度
    Rcpp::IntegerVector t_charWidth(n);
    ...
    for (std::size_t i = 0; i < maxLengthCells; i++){
      ...
      for (j = 0; j < n; j++) {
        ...
        t_charWidth[j] = charWidth[i*n + j];
      }
      res +=  tableExpand_cpp(newCells, colsWidth, t_charWidth, justify, sepCols, style);
    ...
    for (i = 0; i < cells.size(); i++){
      res += format_cpp(as<std::string>(cells[i]), as<std::string>(justify[i]), colsWidth[i], charWidth[i]);
